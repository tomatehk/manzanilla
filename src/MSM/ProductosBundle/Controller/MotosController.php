...
//entidad
use MSM\ProductosBundle\Entity\Rmotos;
use MSM\ProductosBundle\Form\RmotosType;
...
{	
	...
	*/
	...
	{	
		...
		return $this->render('ProductosBundle:Motos:agregar-producto-motos.html.twig', array('form' => $form->createView()));
	}
	...
	public function agregarCheckAction(Request $request)
	{
		...
		return $this->redirectToRoute('productos_motos_agregar');
	}

	private function crearFormProductos(Rmotos $entity)
	{
		$form = $this->createForm(new RmotosType(), $entity, array(
			'action' => $this->generateUrl('productos_agregar_motos_check'),
			'method' => 'POST'
		));
	...
	*/
	public function listaAction(Request $request)
	{
		...
		if(!empty($request->get('producto'))){
			...
			//consultamos
			$consulta = $em->createQuery("SELECT u FROM ProductosBundle:Rmotos u WHERE u.producto LIKE :producto OR u.id = :id");
		...
		if(!$listaProductos){
			return $this->render('ProductosBundle:Motos:no-productos-motos.html.twig');
		}
		...
		return $this->render('ProductosBundle:Motos:lista-productos-motos.html.twig', array('pagination' => $pagination));
	}
	/*
	*Fin lista productos
	*/
	...
	*/
	public function editarAction($id ,Request $request)
	{
		...
		return $this->render('ProductosBundle:Motos:editar-producto-motos.html.twig', array('form' => $form->createView()));
	}

	private function crearFormProductosEditar(Rmotos $entity, $id)
	{
		$form = $this->createForm(new RmotosType(), $entity, array(
			'action' => $this->generateUrl('productos_motos_editar', array('id' => $id)),
			'method' => 'POST'
		));
